...
export default function DimensionCard({
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2 shrink-0 ml-3">
          {whoToAsk.length > 0 && (
            <div className="relative">
              <button
                type="button"
                onClick={() => setShowWhoToAsk((v) => !v)}
                aria-label="Who can help answer this?"
                aria-expanded={showWhoToAsk}
                className="flex items-center justify-center w-5 h-5 rounded-full text-xs font-semibold leading-none"
                style={{
              ...
              {showWhoToAsk && (
                <div
                  className="absolute right-0 mt-1 z-20 w-64 rounded-lg p-3 shadow-lg text-left"
                  style={{
                    backgroundColor: NHS_COLOURS.white,
                ...
                >
                  <p
                    ...
                    style={{ color: NHS_COLOURS.darkText }}
                  >
                    Who can help you answer this?
                  </p>
                  <ul
                    className="text-xs space-y-0.5 list-disc pl-4"
                    style={{ color: NHS_COLOURS.secondaryText }}
                  >
                    {whoToAsk.map((role, i) => (
                      <li key={role}>
      ...
      {onJustificationChange && (
        <div className="mb-3">
          <textarea
            ...
            rows={2}
            placeholder="Brief reasoning for this score (optional but recommended)"
      ...
      {/* What good looks like + tools (readiness dimensions only) */}
      {(bestPractice || resources.length > 0) && (
        <div
        ...
        >
          ...
          >
            {showResources
              ? "Hide guidance"
              : `What good looks like, and tools to help${resources.length > 0 ? ` (${resources.length})` : ""}`}
          </button>
          {showResources && (
              ...
              {resources.length > 0 && (
                <div>
                  <p
                    ...
                    style={{ color: NHS_COLOURS.darkText }}
                  >
                    Tools to help build this readiness
                  </p>
