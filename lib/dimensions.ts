...
export const bestPracticeByDimension: Record<string, string> = {
  ...
  R6: "The tool integrates with existing clinical systems using established standards (FHIR UK Core, recognised APIs) rather than bespoke connections that become single points of failure. There is named technical support with a clear escalation path, defined processes for handling failure, degradation, or unavailability, and a budget that accounts for the full lifecycle — monitoring, updates, and decommissioning — not just go-live.",
  R7: "The affected workflow has been mapped end to end (for example using the QSIR tools), and the people whose work changes have been engaged early enough to shape the rollout rather than have it done to them. There is a named change lead with protected time, a phased rollout with explicit success criteria and a withdrawal trigger, and — where the tool displaces or reshapes roles — workforce implications planned with HR and staff-side representatives.",
